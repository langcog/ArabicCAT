...
save(file=here("data/Arabic_LD.Rds"), "res")
...
hiLDvio_words = coefs_2pl[which(hiLDvio>10),]$definition # 682 !

modLDvio = get_LD_violations(res)
modLDvio_words = coefs_2pl[which(modLDvio>10),]$definition # 692 words have moderate LD ... 


# ToDo: try just removing the few items with many violations (e.g., 1 item has 33..),
...
save(file=here("data/fr_ws_wg_2pl_itemfits.Rds"), "itfit2pl") #  "itfit2pl_x2"
```

```{r, echo=F}
load(here("data/fr_ws_wg_2pl_itemfits.Rds"))
#bad_items2pl = which(itfit2pl$p.S_X2 < .05) # 0!
#bad_items2pl = which(itfit2pl$p.S_X2 < .01) # 0 with p<.01 
bad_items2pl = which(itfit2pl$p.X2 < .001) # 48
...
```{r, include=F, echo=F}
kable(matrix(itfit2pl$item[bad_items2pl][1:48], ncol=4)) %>% 
