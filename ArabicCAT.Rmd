---
title: "Arabic Short Forms and CDI-CAT"
author: "George & Mike"
date: "6/23/2022"
...
This markdown documents data import of JISH and Alroqi datasets with the goals of :
1. Wordbank import
2. Norm creation
3. Short form creation
4. CAT creation
...
# sheets <- readxl::excel_sheets(path="raw_data/JISH - CDI Data_updated.xlsx")

raw <- read_xlsx(path="raw_data/JISH - CDI Data_updated.xlsx", 
                 sheet="JACDI Data")

notes <- read_xlsx(path="raw_data/JISH - CDI Data_updated.xlsx", 
                   sheet="Notes")

codebook <- read_xlsx(path="raw_data/JISH - CDI Data_updated.xlsx", 
                      sheet="Codebook")
```

Notes included in the datafile. 
...
items <- bind_cols(definition=arabic_names[8:1984], uni_lemma=col_names[8:1984])
...
d_demo = raw[3:nrow(raw),c(1:7, 1985:2013)] # last three columns are na.21 - na.23 - check and drop these?
...
save(file="cached_data/JACDI_arabic_data.Rds", "prod_voc", "comp_voc", "d_demo")
```

Check prod. 

```{r}
ggplot(d_demo, aes(x = age, y = production)) + 
...
ggplot(d_demo, aes(x = age, y = comprehension)) + 
  geom_jitter(width = .2, height = 0, alpha = .5) + 
...
```
Our tentative conclusion is that production up to 30 months should be useable but we are worried about both comprehension data and 31-36 month production data. 
...
```{r eval=FALSE}
raw_wg <- read_xlsx(path="raw_data/Alroqi et al. 2020 - Saudi CDI - WG - updated.xlsx", sheet="Saudi CDI - WG")
...
There are `r length(intersect(new_wg_names, names(raw2)))` matching names for items, but there are also many that don't match between these data and the JISH data: `r setdiff(new_wg_names, names(raw2))`.
...
```{r}
ggplot(raw_wg, aes(x = computed_age_mo, y = `Total understood words (8-16 mths)`)) + 
  geom_point(alpha = .5) + 
  xlab("Total comprehension") + 
  ylab("Age (months)")
...
```
```{r}
ggplot(raw_wg, aes(x = computed_age_mo, y = `Total produced words (8-16 mths)`)) + 
  geom_point()
```

### WS data

Data format is difficult -- looks like each row (child) lists the items they know in each category, but NOT in the same columns...need to either iterate over each row and try to match the columns, or reformat by hand..


```{r}
raw_ws <- read_xlsx(path="raw_data/Alroqi et al. 2020 - Saudi CDI - WS - updated.xlsx", 
                    sheet="Saudi CDI - WS") # 243 subjects
```
```{r}
ggplot(raw_ws, aes(x = computed_age_mo, y = `Total produced words (17-36 mths)`)) +
  geom_point(alpha = .5) 
```
Deal with weird data format. There are 896 columns on the form. 

```{r}
words <- unique(unlist(flatten(array(raw_ws[1:242,25:921]))))
words <- words[!is.na(words)]

ws <- tibble(subid = raw_ws$`#`) 
ws[,words ] <- NA
...
```{r}
alroqi_full_prod <- left_join(processed_ws, 
                          raw_ws |>
                            rename(subid = `#`, 
                                   gender = Gender, 
                            ...
                            select(subid, 
                                   age_mo, 
                                   gender)) 
alroqi_full_prod$source <- "Alroqi"
```
...
anames <- names(alroqi_prod)
jnames_nop <- str_replace(jnames, "\\.p", "")
names(jish_prod) <- jnames_nop

jish_full_prod <- left_join(jish_demo, jish_prod)
jish_full_prod <- mutate(jish_full_prod, across(all_of(jnames_nop[jnames_nop != "subid"]), 
                                                as.numeric)) 
```
...
ggplot(full_prod, aes(x = age_mo, y = total, col = source)) + 
  geom_jitter(width = .2, height = 0, alpha = .5)
```
Let's filter various kids we don't think should be in here. 
...
# double-check their production totals

table(d_demo$age) %>% kable(col.names=c("Age","N"))
